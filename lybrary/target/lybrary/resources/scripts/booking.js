$(document).ready(function(){
	...
	$(".modifyit").click(function(){
		...
		var id = $(this).attr("id");
		$.post("/modifyItem", {documentId:id, title:object.find(".books_inputs_title").val(), author:object.find(".books_inputs_author").val(), publisher:object.find(".books_inputs_publisher").val(), year:object.find(".books_inputs_year").val(), edition:object.find(".books_inputs_edition").val(), note:object.find(".books_inputs_note").val()}, function(result){
			alert(result);
		});
	});
	$("#add_save_book").click(function(){
		$.post("/addDoc", {title:$("#add_book_title").val(), author:$("#add_book_author").val(), publisher:$("#add_book_publisher").val(), year:$("#add_book_year").val(), edition:$("#add_book_edition").val(), note:$("#add_book_note").val()}, function(result){
			alert(result);
		});
	});
	$("#add_save_av").click(function(){
		$.post("/addDoc", {title:$("#add_av_title").val(), author:$("#add_av_author").val()}, function(result){
			alert(result);
		});
	});
